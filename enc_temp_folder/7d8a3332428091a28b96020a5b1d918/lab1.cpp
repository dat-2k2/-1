...
int main()
{
	...
	// адрес double данной: 5f f6 46 37 dd 9a bf 3f; 
	//читать лево-право. 
	// 3fb = (0)(1111111011), где 0 - знак, 1111111011 - степень (unsigned 11 bits), остальный - мантисса. 
	d = 2.7;
	// адрес: 9а 99 99 99 99 99 05 40. 
	// само точно - 40 05 99 99 99 99... но d выражено 8 bytes итак округлено до 9a. 
	float  f = 0.1234567890123456789123456789;
	// адрес float данной: ea d6 fc 3d - float 4 bytes
	// float: 3df = (0)(01111011)(111, 0 - знак, 01111011 - степень (unsigned 6 bits), остальный - мантисса. 
	
	f = 2.7;
	// адрес float number: cd cc 2c 40 - 4 bytes. 
	
	// отображение на памяти f и d разны потому что bias разны. Степень равно 4, но разные bias 
	// (127 для float 32 bits и 1023 для double 64 bits). 

	d = 1.; //00 00 00 00 00 00 f0 3f
	d = 0.999999999999999999999999999999999;
	// тоже 00 00 00 00 00 00 f0 3f  
	// одиноковое отображение потому что 0.9999999... (10) = 0.11111...(2) = 1. 
	...
	stop
		...
		//личное замечание:
		// дано a - вещественный, на десятичном. Тогда число с плавающей запятой имеет как, что:
		// первый bit - знак (0/1), 6 (f) или 11 (d) - степень и остальные - мантисса. 
		// формула: a = (1-2b)*2^[e - 2^(k-1) - 1]*мантисса, где b - десятичное значение первого bit,
		// e - степени на usigned int форме, k - число bits для выражения степени. 
		// 
		// Algorithm: (float to hex IEEE )
		//			  - Перенести число на бинарной  
		//			  - Найти число e, такой что, для числа умноженного на 2^e (бинарной) останется только 
		//				1 единица 1 перед запятой. Добавить е и 2^7-1 - bias, получаем степени, потом выражит его по 8bits.
		//			  - Остальный, удалить 1 перед запятой и выражит остальный на 23 bits. 
		//			  - Перенести на hex. 
		// 
		// Почему минус 2^(k-1)-1? потому что для десятичной, степень вероятно отрацительная,
		// итак для бинарной, нужно добавить bias (2^(k-1)-1) чтобы перенести [-2^(k-1)+1,2^(k-1)] на [0,2^k-1].  
		// **************************************************************
	...
	double dDouble3 = (float)iInt / 3;	// (3)
